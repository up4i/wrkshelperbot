...
_PAGES = {
    ...
    "setup": (
        ...
        "💸 *Admin Economy* _(Owner and EcoAdmins)_\n"
    ...
    "gifts": (
        ...
        "🏪 *Shop & Market*\n"
        "`/shop` · `/market` — browse all collections with inline pages\n"
        "`/shop <collection>` — see models & prices for one collection\n"
        "└ e.g. `/shop scared cat` or `/shop scared_cat`\n\n"
        "🛒 *Buying & Selling*\n"
        "`/buy <collection> <#>` — buy that numbered gift from Rift\n"
        "└ e.g. `/buy scared cat 12`\n"
        "`/sell <collection> <#>` — sell back (80% of market price)\n\n"
        "🎒 *Inventory*\n"
        "`/inventory` · `/inv` — browse your gifts with inline pages\n"
        "`/gift <collection> <#> [bg]` — flex a gift in chat\n"
        "`/pin` — pin a gift to your profile (shows on `/p`)\n\n"
        "🤝 *Trading*\n"
        "`/offer @user <amount> for <collection> <#>`\n"
    ...
    "economy": (
        ...
        "🏆 *Leaderboard*\n"
        "`/leaderboard` · `/lb` — leaderboard with tab buttons\n"
        ...
        "🎰 *Gambling* — use `all`, `50k`, or `2.5m` amounts (min 10 WRK$)\n"
